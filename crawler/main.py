...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--url", default=None, help="推送接口地址")
    parser.add_argument(
        "--wrap-payload",
        action="store_true",
...
def main() -> None:
    ...
    try:
        ...
        if args.push:
            target_url = args.url or (DEFAULT_RECOMMEND_URL if args.recommend else DEFAULT_SYNC_URL)
            logger.info("推送数据到接口：%s", target_url)
            success = push_to_java(
                output_data if args.recommend else foods,
