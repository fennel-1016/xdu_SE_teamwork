...
def build_sync_payload(foods: List[Dict]) -> List[Dict]:
    """转换为后端 SyncFoodRequest 支持的 JSON 数组字段。"""
    payload = []

    for food in foods:
        payload.append(
            {
                "name": food.get("name") or food.get("food_name"),
                "price": food.get("price"),
                "canteen": food.get("canteen") or food.get("canteen_name") or food.get("category") or "未知食堂",
                "window": food.get("window") or food.get("window_no") or "未知窗口",
            }
        )
